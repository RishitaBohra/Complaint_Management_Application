...
class ViewCompletedComplaints extends StatefulWidget {
  const ViewCompletedComplaints({super.key});

  @override
  State<ViewCompletedComplaints> createState() => _ViewCompletedComplaintsState();
}

class _ViewCompletedComplaintsState extends State<ViewCompletedComplaints> {
  ...
  Future<void> fetchComplaints() async {
    try {
      final response = await http.get(
        Uri.parse("${baseUrl}api/complaints/by-staff/$staffId"), // ✅ Use baseUrl and dynamic staffId
      ...
      if (response.statusCode == 200) {
        ...
        if (data != null && data.isNotEmpty) {
          ...
          debugPrint("ℹ️ No completed complaints found.");
          setState(() => isLoading = false);
        }
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading
          ...
          : complaints.isEmpty
              ? const Center(child: Text("No completed complaints found."))
              : ListView.builder(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(" Complainant Name: ${complaint.name ?? 'N/A'}"),
