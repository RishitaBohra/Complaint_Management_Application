...
import 'dart:convert';
import 'package:amity_university/ServiceProvider/Presentation/UI/auth/serviceproviderpin.dart';
...
class _ServiceProviderLoginScreenState
  ...
  Future<void> loginWithPhone() async {
    ...
    try {
      ...
      debugPrint("📦 Staff Login Response: $data");

      if (response.statusCode == 200 && data['pin'] != null) {
        SharedPreferences prefs = await SharedPreferences.getInstance();
        await prefs.setString('loggedInProviderMobile', phone);
        await prefs.setString('role', 'service_provider');
        debugPrint("✅ Saved loggedInProviderMobile: $phone");

        if (!mounted) return;
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => ServiceProviderOTPScreen(
              mobile: phone,
